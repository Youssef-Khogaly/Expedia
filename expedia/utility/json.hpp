...
namespace json
{
  ...
  public:
	...
	{
	  return
	      object ? object->begin () : typename Container::const_iterator ();
	}
	typename Container::const_iterator
	end () const
	{
    ...
    template<typename T>
      typename enable_if<is_same<T, bool>::value, JSON&>::type
      operator= (T b)
      {
    ...
    template<typename T>
      typename enable_if<is_floating_point<T>::value, JSON&>::type
      operator= (T f)
      {
	...
	*Internal.String = string (s);
    ...
    bool
    ToBool (bool &ok) const
    {
      ok = (Type == Class::Boolean);
      return ok ? Internal.Bool : false;
    }

    JSONWrapper<map<string, JSON>>
    ObjectRange ()
    {
      if (Type == Class::Object)
    ...
    JSONConstWrapper<map<string, JSON>>
    ObjectRange () const
    {
      if (Type == Class::Object)
    ...
    string
    dump (int depth = 1, string tab = "  ") const
    {
      string pad = "";
    ...
    friend std::ostream&
    operator<< (std::ostream&, const JSON&);
  ...
  {
    JSON
    parse_next (const string&, size_t&);

    void
    consume_ws (const string &str, size_t &offset)
    {
    ...
    JSON
    parse_object (const string &str, size_t &offset)
    {
    ...
    JSON
    parse_array (const string &str, size_t &offset)
    {
    ...
    JSON
    parse_string (const string &str, size_t &offset)
    {
      JSON String;
      string val;
    ...
    JSON
    parse_number (const string &str, size_t &offset)
    {
      JSON Number;
      string val, exp_str;
      char c;
    ...
    JSON
    parse_bool (const string &str, size_t &offset)
    {
      JSON Bool;
    ...
    JSON
    parse_null (const string &str, size_t &offset)
    {
      JSON Null;
    ...
    JSON
    parse_next (const string &str, size_t &offset)
    {
      char value;
      consume_ws (str, offset);
      value = str[offset];
